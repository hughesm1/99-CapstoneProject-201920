...
class DriveSystem(object):
    ...
    def go_until_distance_is_within(self, delta, inches, speed):
        ...
        from the object.
        """
        while self.sensor_system.ir_proximity_sensor.get_distance() >= delta:
            self.go_forward_until_distance_is_less_than(delta, speed)
...
class ArmAndClaw(object):
    ...
    def calibrate_arm(self):
        ...
        self.motor.turn_on(-100)
        while True:
            if abs(self.motor.get_position()) >= zero_mark:
                self.motor.turn_off()
                self.motor.reset_position()
                break
    ...
    def lower_arm(self):
        ...
        self.motor.turn_on(-100)
        while True:
            amount_away_from_zero = self.motor.get_position()
